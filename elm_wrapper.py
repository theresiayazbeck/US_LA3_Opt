"""
Optimization wrapper for ELM.
These functions provide the interface between the optimization tool and ELM
...
#subprocess.call("/global/cscratch1/sd/yazbeck/CA_TVC_Trans/case.submit")
...
def get_model_obs(obsfile, ex_settings, model_settings, parameters, trial_index):
    ...
    AllModVarName = ex_settings["experiment"]["model_var"]
    ...
    #modeldf["CH4_SURF_AERE_TOT"] = np.multiply(AllModelVar[:,0],AllModelVar[:,2])+np.multiply(AllModelVar[:,1],(1-AllModelVar[:,2]))
    modeldf["GPP"] = AllModelVar[:,0]
    ...
    modeldf_not_nans = modeldf["GPP"].loc[not_nans]
...
class ELMWrapper(BaseWrapper):
    ...
    def run_model(self, trial: Trial):
        ...
        cmd = (f"./create_newcase -case {case_dir} -compset I20TRGSWCNPECACNTBC -mach cori-knl -compiler intel -project m2467 -res 1x1_brazil")
        ...
        OrigParamFile = working_dir + experiment_name + '/ParamInput/clm_params.cbgc.c07292018.nc'
