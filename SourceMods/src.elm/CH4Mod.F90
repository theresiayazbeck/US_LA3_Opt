...
#include "shr_assert.h"
  ...
  type, private :: CH4ParamsType
     ...
     real(r8), pointer :: MeasAereResis=> null()         ! measured aerenchyma resistance (s/m)
...
contains
  ...
  subroutine readCH4Params ( ncid )
    ...
    allocate(CH4ParamsInst%MeasAereResis       )
    ...
    if ( .not. use_aereoxid_prog ) then
     ...
     tString='MeasAereResis'
     call ncd_io(trim(tString),tempr, 'read', ncid, readvar=readv)
     if ( .not. readv ) call endrun(msg=trim(errCode)//trim(tString)//errMsg(__FILE__, __LINE__))
     CH4ParamsInst%MeasAereResis=tempr
    ...
    associate(                                                     &
      ...
      do j=1,nlevsoi
         do fc = 1, num_methc
            ...
            write(iulog,*)'sat,c,j,partition_z',sat,c,j,partition_z
      ...
      if (.not. lake) then
         ...
         do fp = 1, num_methp
            ...
            call SiteOxAere(is_vegetated, watsat(c,1:nlevsoi), h2osoi_vol(c,1:nlevsoi), t_soisno(c,1:nlevsoi), & 
                 ...
                 z(c,1:nlevsoi), dz(c,1:nlevsoi), sat, CH4ParamsInst%MeasAereResis,& 
            ...
            do j=1,nlevsoi
               ...
               ch4_aere_depth (c, j) = 0 !pft weight in col.
  ...
  subroutine SiteOxAere(is_vegetated, & 
    ...
    real(r8) :: MeasAereResis   ! resistance to methane flux measured using chamber measurements at leaf level (This is per leaf area) (s/m)
    ...
    do j=1,nlevsoi   
       PftAereParam = PftAereParam + rootr(j)/z(j)
    enddo

    PftAereParam = MeasAereResis*PftAereParam
    ...
    do j=1,nlevsoi
       ...
       if (j > jwt .and. t_soisno(j) > tfrz .and. is_vegetated) then
          ...
          aerecond = (1._r8/(z(j)*PftAereParam))*rootr(j)*elai
